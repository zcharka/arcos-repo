...
class InstallationWidget(Gtk.Box):
    ...
    def _get_mount_root_command(self):
        ...
        mkdir -p /tmp/arcos_installer/root
        ...
        case "$FS_TYPE" in
            btrfs)
                ...
                if [ ! -z "$SUBVOL" ] && [ "$SUBVOL" != "" ]; then
                    ...
                    mount -t btrfs -o subvol="$SUBVOL" "$ROOT_DEVICE" "/tmp/arcos_installer/root"
                elif [ ! -z "$SUBVOLID" ]; then
                    echo "Mounting Btrfs with subvolume ID: $SUBVOLID"
                    mount -t btrfs -o subvolid="$SUBVOLID" "$ROOT_DEVICE" "/tmp/arcos_installer/root"
                else
                    echo "Mounting Btrfs root (no subvolume specified)"
                    mount -t btrfs "$ROOT_DEVICE" "/tmp/arcos_installer/root"
                fi
                ...
                if [ $MOUNT_RESULT -eq 0 ]; then
                    ...
                    while IFS= read -r line; do
                        ...
                        if [ ! -z "$SUBVOL_NAME" ] && [ "$SUBVOL_NAME" != "" ]; then
                            ...
                            mkdir -p "/tmp/arcos_installer/root$MOUNT_POINT"
                            mount -t btrfs -o subvol="$SUBVOL_NAME" "$ROOT_DEVICE" "/tmp/arcos_installer/root$MOUNT_POINT"
                            if [ $? -eq 0 ]; then
            ...
            ext2|ext3|ext4)
                ...
                mount -t "$FS_TYPE" "$ROOT_DEVICE" "/tmp/arcos_installer/root"
                MOUNT_RESULT=$?
                ;;

            xfs)
                echo "Mounting XFS filesystem"
                mount -t xfs "$ROOT_DEVICE" "/tmp/arcos_installer/root"
                MOUNT_RESULT=$?
                ;;

            f2fs)
                echo "Mounting F2FS filesystem"
                mount -t f2fs "$ROOT_DEVICE" "/tmp/arcos_installer/root"
                MOUNT_RESULT=$?
                ;;

            jfs)
                echo "Mounting JFS filesystem"
                mount -t jfs "$ROOT_DEVICE" "/tmp/arcos_installer/root"
                MOUNT_RESULT=$?
                ;;

            reiserfs)
                echo "Mounting ReiserFS filesystem"
                mount -t reiserfs "$ROOT_DEVICE" "/tmp/arcos_installer/root"
                MOUNT_RESULT=$?
                ;;
            ...
            ntfs|ntfs-3g)
                ...
                mount -t ntfs3 "$ROOT_DEVICE" "/tmp/arcos_installer/root" 2>/dev/null
                if [ $? -ne 0 ]; then
                    echo "ntfs3 driver failed, trying ntfs-3g..."
                    mount -t ntfs-3g "$ROOT_DEVICE" "/tmp/arcos_installer/root"
                fi
                MOUNT_RESULT=$?
                ;;
            ...
            *)
                ...
                mount "$ROOT_DEVICE" "/tmp/arcos_installer/root"
                MOUNT_RESULT=$?
                ;;
        esac
    ...
    def _get_mount_boot_command(self):
        ...
        # Create mount point
        mkdir -p /tmp/arcos_installer/root/boot
        ...
        case "$FS_TYPE" in
            btrfs)
                if [ ! -z "$BOOT_SUBVOL" ] && [ "$BOOT_SUBVOL" != "" ]; then
                    ...
                    mount -t btrfs -o subvol="$BOOT_SUBVOL" "$BOOT_DEVICE" "/tmp/arcos_installer/root/boot"
                else
                    echo "Mounting Btrfs boot (no subvolume)"
                    mount -t btrfs "$BOOT_DEVICE" "/tmp/arcos_installer/root/boot"
                fi
                ;;

            ext2|ext3|ext4)
                echo "Mounting ext boot filesystem"
                mount -t "$FS_TYPE" "$BOOT_DEVICE" "/tmp/arcos_installer/root/boot"
                ;;

            xfs)
                echo "Mounting XFS boot filesystem"
                mount -t xfs "$BOOT_DEVICE" "/tmp/arcos_installer/root/boot"
                ;;
            ...
            ntfs|ntfs-3g)
                ...
                mount -t ntfs3 "$BOOT_DEVICE" "/tmp/arcos_installer/root/boot" 2>/dev/null || \
                mount -t ntfs-3g "$BOOT_DEVICE" "/tmp/arcos_installer/root/boot"
                ;;

            *)
                echo "Filesystem type: $FS_TYPE - attempting auto-detect mount"
                mount "$BOOT_DEVICE" "/tmp/arcos_installer/root/boot"
                ;;
        esac
        ...
        if [ $? -eq 0 ]; then
            ...
            if mountpoint -q /tmp/arcos_installer/root/boot; then
                echo "Boot mount point verification successful"
                df -h /tmp/arcos_installer/root/boot

                # Check for EFI files
                if [ -d "/tmp/arcos_installer/root/boot/EFI" ]; then
                    echo "EFI directory found - this is an EFI boot partition"
                elif [ -d "/tmp/arcos_installer/root/boot/grub" ] || [ -d "/tmp/arcos_installer/root/boot/grub2" ]; then
            ...
            if [ -d "/tmp/arcos_installer/root/boot" ]; then
                echo "Boot directory exists on root partition"
                ls -la /tmp/arcos_installer/root/boot/ 2>/dev/null | head -5
            fi
    ...
    def _get_mount_extra_partitions_command(self):
        ...
        grep -v "^#" "$FSTAB_FILE" | awk '$2 != "/" && $2 != "/boot" && $3 != "swap" && $2 != "" {print $0}' | while read -r line; do
            ...
            TARGET_DIR="/tmp/arcos_installer/root$MOUNTPOINT"
            mkdir -p "$TARGET_DIR"
    ...
    def _get_copy_config_command(self):
        ...
        TARGET_DIR="/tmp/arcos_installer/root"
...
DESKTOP
    ...
    def _get_copy_kernel_command(self):
        ...
        return """
        TARGET_DIR="/tmp/arcos_installer/root/boot"
        mkdir -p "$TARGET_DIR"
    ...
    def _get_create_swap_command(self, size_mb):
        ...
        return r"""
        set -u
        ROOT_MNT="/tmp/arcos_installer/root"
    ...
    def start_installation(self, loop_device="/dev/loop0"):
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "mkdir", "-p", "/tmp/arcos_installer/rootfs", "/tmp/arcos_installer/root"],
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "mount", loop_device, "/tmp/arcos_installer/rootfs"],
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "mkdir", "-p", "/tmp/arcos_installer/root/boot"],
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "rsync", "-aAXv", "--info=progress2", "/tmp/arcos_installer/rootfs/", "/tmp/arcos_installer/root/"],
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "bash", "-c", "echo 'Files copied:' && find /tmp/arcos_installer/root -type f | wc -l"],
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "rm", "/etc/sudoers.d/g_wheel"],
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "bash", "-c", "/language.sh"],
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "bash", "-c", "/setup_timezone.sh"],
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "bash", "-c", "/setup_keyboard.sh"],
        ...
        steps.append(InstallationStep(
            label="Adding user",
            command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "bash", "-c", "/add_users.sh"],
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "bash", "-c", "/remove_ucode.sh"],
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "bash", "-c", "/bootloader.sh"],
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "bash", "-c", "/remove_gpu.sh"],
        ...
        if selected_flatpaks:
            steps.append(InstallationStep(
                ...
                command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "flatpak", "update", "--appstream"],
            ...
            steps.append(InstallationStep(
                ...
                command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "flatpak", "install"] + selected_flatpaks + ["--assumeyes"],
                description="Installing Flatpak apps and support for AppImage",
                weight=5.0,
                critical=False
            ))
            ...
            if os.path.exists(removal_config):
                ...
                if removed_pkgs:
                    ...
                    steps.append(InstallationStep(
                        ...
                        command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "pacman", "-Rns", "--noconfirm"] + removed_pkgs,
        ...
        steps.append(InstallationStep(
            label="Cleaning out rootfs",
            command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "bash", "-c", "rm /*.sh"],
            description="Cleaning out rootfs from LiveISO's config and applying post-install scripts",
            weight=1.0,
            critical=False
        ))
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "bash", "-c",
                     ...
                     'chmod +x /usr/local/bin/arcos-snapshot 2>/dev/null; '
                     '/usr/local/bin/arcos-snapshot configure; '
                     'mv -f /etc/pacman.d/hooks/00-arcos-snapshot.hook.inactive '
                     '/etc/pacman.d/hooks/00-arcos-snapshot.hook 2>/dev/null || true; '
                     'else '
                     'rm -f /etc/pacman.d/hooks/00-arcos-snapshot.hook.inactive 2>/dev/null || true; '
                     'fi'],
        ...
        steps.append(InstallationStep(
            ...
            command=["sudo", "bash", "-c",
                    "umount -R /tmp/arcos_installer/root 2>/dev/null; "
                    "umount -R /tmp/arcos_installer/rootfs 2>/dev/null; "
                    "true"],
    ...
    def _cleanup_mounts_async(self):
        ...
        def _run():
            mount_points = (
                "/tmp/arcos_installer/root",
                "/tmp/arcos_installer/rootfs",
            )
