...
detect_other_os() {
    ...
    if [ "$BOOT_MODE" = "uefi" ] && [ -n "$ESP_PATH" ] && [ -d "$ESP_PATH/EFI" ]; then
        ...
        for dir in "$ESP_PATH/EFI"/*; do
            ...
            case "$dirname" in
                systemd|BOOT|Boot|refind|tools|ArcOS)
    ...
    if [ "$BOOT_MODE" = "uefi" ] && command -v efibootmgr &>/dev/null; then
        ...
        if echo "$efi_output" | grep -qiE "Windows|Microsoft|Ubuntu|Manjaro|Debian|Fedora|openSUSE|Mint"; then
            local non_arcos_entries=$(echo "$efi_output" | grep -iE "Windows|Microsoft|Ubuntu|Manjaro|Debian|Fedora|openSUSE|Mint" | grep -v -i "ArcOS")
            if [ -n "$non_arcos_entries" ]; then
                local found_entry=$(echo "$non_arcos_entries" | head -1)
...
EOF
    ...
    if [ -n "$esp_device" ]; then
        ...
        if [ -n "$esp_part_num" ] && [ -n "$esp_disk" ]; then
            ...
            efibootmgr -c -d "$esp_disk" -p "$esp_part_num" -L "ArcOS" -l "\\EFI\\systemd\\systemd-bootx64.efi" >/dev/null 2>&1 || true

            local linux_boot=$(efibootmgr 2>/dev/null | grep "ArcOS" | grep -o '^Boot[0-9A-F]\{4\}' | sed 's/Boot//' | head -1)
...
install_grub() {
    ...
    if [ "$boot_mode" = "uefi" ]; then
        ...
        if ! grub-install --target=x86_64-efi --efi-directory="$esp_path" --bootloader-id=ArcOS; then
...
GRUB_DEFAULT=0
GRUB_TIMEOUT=${GRUB_TIMEOUT_VALUE:-5}
GRUB_DISTRIBUTOR="ArcOS"
...
DISTRIB_DESCRIPTION="ArcOS"
EOF

    print_msg "Cleaning up boot directory..."
    # Install ArcOS GRUB theme
    print_msg "Installing ArcOS GRUB theme..."
    mkdir -p /boot/grub/themes/arcos
    cp -r /tmp/installer_config/grub_theme/* /boot/grub/themes/arcos/ 2>/dev/null || true
    echo "GRUB_THEME=\"/boot/grub/themes/arcos/theme.txt\"" >> /etc/default/grub
...
"Boot ArcOS"                 "$root_base quiet splash"
"Boot ArcOS (terminal)"      "$root_base systemd.unit=multi-user.target"
...
main() {
    ...
    # ArcOS always uses GRUB - regardless of BIOS/UEFI, regardless of what
    # bootloader_choice says, regardless of other OSes present. The only
    # thing OS detection still affects is the boot menu timeout: instant
    # boot into ArcOS when it's the only OS, a normal timeout so the user
    # can actually pick when something else is present.
    OTHER_OS=$(detect_other_os)
    print_debug "Detection result: OTHER_OS='$OTHER_OS'"

    if [ "$OTHER_OS" = "true" ]; then
        GRUB_TIMEOUT_VALUE=5
    else
    ...
    print_msg "═══════════════════════════════════════════"
    print_msg ">>> INSTALLING GRUB ($BOOT_MODE, always) <<<"
    print_msg "GRUB_TIMEOUT will be $GRUB_TIMEOUT_VALUE (other OS detected: $OTHER_OS)"
