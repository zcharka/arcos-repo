...
The **ArcOS Installer** is a modern, user-friendly Linux installer built with **Python 3**, **GTK 4**, and **Libadwaita**. It provides a guided step-by-step process to install the ArcOS operating system (or other customized distributions) onto a target machine.
...
### Directory Structure
The application source is located in `src/usr/share/arcos-installer/`.
...
| **DEPicker** | `de_picker_widget.py` | Selection screen for Desktop Environment (e.g., ArcOS vs. Kinexin). Writes selection to `/tmp/installer_config`. |
...
steps.append(InstallationStep(
    ...
    command=["sudo", "/usr/share/arcos-installer/custom-setup.sh"],
...
1.  **Create your script** in the source directory (e.g., `src/usr/share/arcos-installer/myscript.sh`).
2.  **Ensure it's executable** (`chmod +x myscript.sh`).
3.  **Include it in the build** (ensure it's installed to `/usr/share/arcos-installer/` on the live system).
...
1.  Copy the script to the new root first:
    ...
    steps.append(InstallationStep(
        ...
        command=["sudo", "cp", "/path/to/myscript.sh", "/tmp/arcos_installer/root/tmp/"],
...
2.  Run it via `arch-chroot` (or standard `chroot`):
    ...
    steps.append(InstallationStep(
        ...
        command=["sudo", "arch-chroot", "/tmp/arcos_installer/root", "/bin/bash", "/tmp/myscript.sh"],
